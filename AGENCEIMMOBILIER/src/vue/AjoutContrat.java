...
public class AjoutContrat extends JFrame {
    ...
    public AjoutContrat(int idBien) {
        this.idBien = idBien;
        ...
        JLabel titleLabel = new JLabel("Nouveau contrat");
    ...
    private void ajouterContrat() {
        try {
            ...
            String dateC = dateField.getText().trim();
            String typeContrat = (String) typeComboBox.getSelectedItem();
            float prixC = Float.parseFloat(trancheField.getText().trim());

            if (clientNom.isEmpty() || agentNom.isEmpty() || dateC.isEmpty()) {
            ...
            boolean success = controller.ajouterContrat(dateC, typeContrat, prixC, idClient, idBien);

            if (success) {
                JOptionPane.showMessageDialog(this, "Contrat ajouté avec succès !");
                dispose();
            } else {
                JOptionPane.showMessageDialog(this, "Erreur lors de l'ajout du contrat.", "Erreur", JOptionPane.ERROR_MESSAGE);
            }
